...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		// here, I am going to list out my api's along with User permissions
		 ...
		 .antMatchers(HttpMethod.PUT, "/inwardregister/add/{productId}/{godownId}/{supplierId}").hasAuthority("MANAGER")
		 ...
		 .antMatchers(HttpMethod.POST, "/manager/add").hasAnyAuthority("ADMIN")
		 .antMatchers(HttpMethod.POST, "/employee/add/{managerId}").hasAuthority("ADMIN")
		 .antMatchers(HttpMethod.GET, "/outwardregister/report").hasAuthority("ADMIN") // Report API 3:
		 .antMatchers(HttpMethod.GET, "/godown/report").hasAuthority("ADMIN") // Report API 4:
